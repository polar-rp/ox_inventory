...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	lib.print.info('CustomDrop Inventory.Drops: %s', Inventory.Drops)
...
exports('CreateDropFromPlayer', function(playerId)
	...
	lib.print.info('CreateDropFromPlayer Inventory.Drops: %s', Inventory.Drops)
	TriggerClientEvent('ox_inventory:createDrop', -1, dropId, Inventory.Drops[dropId])
...
local function dropItem(source, playerInventory, fromData, data)
	...
	-- Custom drop logic interception
	local item = Items(fromData.name)
	TriggerClientEvent('ox_inventory:startPlacement', source, fromData, data.count, item)
	return false
end

RegisterNetEvent('ox_inventory:completePlacement', function(dropData, count, coords, rotation, model)
	local source = source
	local playerInventory = Inventory(source)
	if not playerInventory then return end

	local success, response = Inventory.RemoveItem(source, dropData.name, count, dropData.metadata, dropData.slot)

	if success then
		local dropId = CustomDrop('Drop', { { dropData.name, count, dropData.metadata } }, coords, nil, nil,
			Player(source).state.instance, model)
		if dropId and Inventory.Drops[dropId] then
			-- Trigger drop creation event manually if not handled by CustomDrop (it might already do it)
			-- CustomDrop calls TriggerClientEvent('ox_inventory:createDrop', ...) internally, so we just need to ensure rotation is applied if possible
			-- However, CustomDrop doesn't take rotation directly usually, let's check CustomDrop signature.
			-- CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)

			-- We might need to update the heading/rotation of the entity on client side or pass it somehow.
			-- Inventory.Drops[dropId] stores coords, instance, model. We can store heading there if needed?
			-- Standard ox_inventory drops don't save rotation usually, they just spawn on ground.
			-- But the user wants placement.

			-- If we want to persist rotation, we'd need to modify CustomDrop logic or 'ox_inventory:createDrop' handler.
			-- For now, let's just spawn it at coords using CustomDrop.
		end
	end
end)
