...
-- Callback for dropping item without opening rightInventory
-- Replicates logic from dropItem function in modules/inventory/server.lua
lib.callback.register('ox_inventory:dropFromInventory', function(source, data)
	...
	if not fromData or fromData.count < count then
		return false
	end

	-- Replicate dropItem logic
	local item = Items(fromData.name)
	TriggerClientEvent('ox_inventory:startPlacement', source, fromData, count, item)
	return false

	-- Original logic commented out/removed by replacement
