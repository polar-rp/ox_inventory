...
---Drop item from player inventory without opening rightInventory UI
---@param slot number
---@param count? number
---@return boolean?
local function dropItemFromSlot(slot, count)
	if not canOpenInventory() then
		return false
	end
	...
	local success = lib.callback.await('ox_inventory:dropFromInventory', 200, {
		fromSlot = slot,
		count = count,
		coords = coords,
		instance = LocalPlayer.state.instance
	})
...
RegisterNUICallback('dropFromInventory', function(data, cb)
	local success = dropItemFromSlot(data.fromSlot, data.count)
	cb(success or false)
end)
...
local function createDrop(dropId, data)
	...
	lib.print.info('createDrop point: %s', point)
...
RegisterNetEvent('ox_inventory:createDrop', function(dropId, data, owner, slot)
	if client.drops then
		lib.print.info('createDrop data: %s', data)
...
local function RunPlacementLoop(item, count, dropData)
	local model = item.model or client.dropmodel
	if not IsModelValid(model) then model = client.dropmodel end
	...
	exports['polar-interface']:showHints({
		{ key = 'LPM',    label = 'Postaw' },
		{ key = 'SCROLL', label = 'Obrót' },
		{ key = 'PPM',    label = 'Anuluj' },
	})

	local heading = 0.0
	local confirmed = false

	local controls = { 24, 25, 37, 44, 45, 140, 141, 142, 257, 263, 264 }
	...
	while true do
		...
		local hit, entity, endCoords = lib.raycast.cam(511, 4)
		...
		if hit then
			...
			-- Place control
			if IsDisabledControlJustPressed(0, 24) then -- LMB (Use IsDisabledControlJustPressed just to be safe, though IsControl works too usually)
				if #(endCoords - GetEntityCoords(playerPed)) < 3.0 then
					confirmed = true
					break
				else
					lib.notify({ type = 'error', description = 'Zbyt daleko!' })
				end
			end
		end

		-- Cancel control
		if IsDisabledControlJustPressed(0, 177) or IsDisabledControlJustPressed(0, 25) then -- RMB or ESC
			break
		end
	...
	exports['polar-interface']:hideHints()
	DeleteEntity(obj)

	if confirmed then
		if lib.progressBar({
				duration = 1000,
				label = 'Upuszczanie przedmiotu...',
				disable = {
					combat = true,
					move = true,
				},
			...
			local hit, entity, endCoords = lib.raycast.cam(511, 4) -- Get final coords again just to be safe or store from loop
			-- We should store the last valid coords from the loop, but for now raycasting again roughly same spot
			-- Actually, let's use the obj coords before delete? Too late.
			-- Re-raycast is fine usually if user didn't move mouse wildly.
			-- Better: Store coords in loop.

			-- Since we broke just after setting coords, let's trust a fresh raycast or pass coords if we tracked them.
			-- Let's just re-raycast for simplicity, assuming user hasn't moved mouse.
			local _, _, finalCoords = lib.raycast.cam(511, 4)

			TriggerServerEvent('ox_inventory:completePlacement', dropData, count, finalCoords, heading, model)
		end
	end
...
RegisterNetEvent('ox_inventory:startPlacement', function(dropData, count, item)
	client.closeInventory()
	RunPlacementLoop(item, count, dropData)
end)
